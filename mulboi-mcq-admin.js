...
function mbPermanentRules(count) {
    ...
    return (
        ...
        `০. ভাষা ও লিপি (script) নিয়ে কঠোর নিয়ম: পেইজের কনটেন্ট প্রকৃতপক্ষে যে ভাষায় লেখা (বাংলা হোক বা ইংরেজি বা অন্য ` +
        `কোনো ভাষা), প্রশ্ন/অপশন/উত্তর/ব্যাখ্যা — সবকিছু অবশ্যই ঠিক সেই একই ভাষায় ও লিপিতে লিখতে হবে; কখনো translate/ ` +
        `convert করা যাবে না। পেইজ যদি বাংলায় হয়, তাহলে ঠিক বাংলা লিপিতে (বাংলা অক্ষর/হরফ ব্যবহার করে, যেমন: অ আ ই উ ক খ ` +
        `গ ঘ) লিখতে হবে — কখনোই দেবনাগরী/হিন্দি লিপি (जैसे: क ख ग घ) ব্যবহার করা যাবে না, এমনকি বাংলা উচ্চারণ হলেও। পেইজ যদি ` +
        `ইংরেজিতে হয়, প্রশ্ন-উত্তর-ব্যাখ্যা ইংরেজিতেই থাকবে — জোর করে বাংলায় অনুবাদ করা নিষিদ্ধ। পেইজের একটা অংশ একরকম ভাষায় ` +
        `আর আরেকটা অংশ অন্য ভাষায় থাকলে (যেমন বাংলা প্যারাগ্রাফের মধ্যে ইংরেজি টার্ম), সেই মিশ্রণ যেভাবে আছে ঠিক সেভাবেই ` +
        `রাখতে হবে — কৃত্রিমভাবে এক ভাষায় "পরিষ্কার" করা যাবে না। প্রতিটি প্রশ্নের ভাষা আলাদাভাবে যাচাই করো (পুরো পেইজের ` +
        `জন্য একবার না) — একই পেইজে ভিন্ন ভিন্ন অংশ থেকে ভিন্ন ভাষায় প্রশ্ন আসাও সঠিক আচরণ। এই নিয়ম অন্য সব নিয়মের চেয়ে ` +
        `অগ্রাধিকার পাবে।\n` +
        `০ক. তথ্যগত নির্ভুলতা (STRICT, সর্বোচ্চ অগ্রাধিকার — quality/smooth output নিশ্চিত করার মূল ভিত্তি): প্রতিটি প্রশ্নের ` +
        `উত্তর ও ব্যাখ্যা অবশ্যই পেইজের প্রকৃত কনটেন্টের সাথে হুবহু সামঞ্জস্যপূর্ণ হতে হবে — অনুমান/আন্দাজ করে ভুল তথ্য বসানো ` +
        `সম্পূর্ণ নিষিদ্ধ। প্রতিটি প্রশ্ন চূড়ান্ত করার আগে নিজে থেকে দুইবার যাচাই করো: (ক) সঠিক উত্তরটি (correct) পেইজের ` +
        `তথ্যের সাথে সত্যিই মিলছে কিনা, (খ) ভুল অপশনগুলো (distractor) সত্যিই ভুল কিনা এবং প্রকৃত সঠিক উত্তরের সাথে গুলিয়ে ` +
        `যাচ্ছে না কিনা। সংখ্যা/তারিখ/সাল/নাম/সূত্র/একক — এই ধরনের নির্দিষ্ট তথ্য পেইজের সাথে অক্ষরে-অক্ষরে/অঙ্কে-অঙ্কে মিলিয়ে ` +
        `লিখতে হবে, নিজের স্মৃতি থেকে "কাছাকাছি মনে হওয়া" মান বসানো যাবে না। পেইজে স্পষ্ট তথ্য না থাকলে এবং প্রশ্ন বানানো ` +
        `সম্ভব না হলে সেই টপিক বাদ দাও, অনিশ্চিত/অনুমাননির্ভর প্রশ্ন বানিয়ে সংখ্যা পূরণ করার চেষ্টা করবে না — একটা সঠিক প্রশ্ন ` +
        `দশটা অনিশ্চিত প্রশ্নের চেয়ে ভালো।\n` +
...
function mbUpdateContext() {
    ...
    if (subName && chName) {
        ctx.innerHTML = '<svg class="emoji-icon" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="#6366F1" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline;vertical-align:-0.125em"><path d="M12 7v14" /> <path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z" /></svg> ' + esc(subName) + ' &gt; <svg class="emoji-icon" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="#6366F1" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline;vertical-align:-0.125em"><path d="M12 7v14" /> <path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z" /></svg> ' + esc(chName);
...
async function mbLoadChapterPdfs() {
    ...
    try {
        ...
        listEl.innerHTML = '<div class="empty-state"><div class="empty-state-icon"><svg class="emoji-icon" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="#F59E0B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline;vertical-align:-0.125em"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3" /> <path d="M12 9v4" /> <path d="M12 17h.01" /></svg>️</div><div class="empty-state-title">লোড ব্যর্থ</div><button class="btn btn-outline btn-sm" style="margin-top:8px" onclick="mbLoadChapterPdfs()">পুনরায় চেষ্টা</button></div>';
    }
}
...
function mbRenderChapterPdfs(pdfs) {
    ...
    if (!listEl) return;
    if (!pdfs.length) {
        listEl.innerHTML = '<div class="empty-state"><div class="empty-state-icon"><svg class="emoji-icon" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="#64748B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline;vertical-align:-0.125em"><path d="M6 22a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.704.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2z" /> <path d="M14 2v5a1 1 0 0 0 1 1h5" /> <path d="M10 9H8" /> <path d="M16 13H8" /> <path d="M16 17H8" /></svg></div><div class="empty-state-title">কোনো PDF নেই</div><div class="empty-state-text">উপরে PDF আপলোড করুন</div></div>';
        return;
    }
        ...
        <div class="pdf-card" id="mbpdf-${p.id}">
            <div class="pdf-card-top">
                ...
                <div class="pdf-card-actions">
                    <button class="act-btn act-edit" title="MCQ সম্পাদনা" onclick="mbOpenMcqPanel(${p.id}, '${esc(p.title)}', '${esc(p.file_url)}')"><svg class='emoji-icon' viewBox='0 0 24 24' width='1em' height='1em' fill='none' stroke='#F59E0B' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' style='display:inline;vertical-align:-0.125em'><path d='M12.659 22H18a2 2 0 0 0 2-2V8a2.4 2.4 0 0 0-.706-1.706l-3.588-3.588A2.4 2.4 0 0 0 14 2H6a2 2 0 0 0-2 2v9.34' /> <path d='M14 2v5a1 1 0 0 0 1 1h5' /> <path d='M10.378 12.622a1 1 0 0 1 3 3.003L8.36 20.637a2 2 0 0 1-.854.506l-2.867.837a.5.5 0 0 1-.62-.62l.836-2.869a2 2 0 0 1 .506-.853z' /></svg></button>
                    <button class="act-btn act-delete" title="মুছুন" onclick="mbDeletePdf(${p.id}, '${esc(p.title)}')"><svg class='emoji-icon' viewBox='0 0 24 24' width='1em' height='1em' fill='none' stroke='#EF4444' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' style='display:inline;vertical-align:-0.125em'><path d='M10 11v6' /> <path d='M14 11v6' /> <path d='M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6' /> <path d='M3 6h18' /> <path d='M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2' /></svg>️</button>
                </div>
            </div>
        </div>`).join('');
}
...
function mbRenderAllPdfs(pdfs) {
    ...
    if (!listEl) return;
    if (!pdfs.length) {
        listEl.innerHTML = '<div class="empty-state"><div class="empty-state-icon"><svg class="emoji-icon" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="#F59E0B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline;vertical-align:-0.125em"><path d="m6 14 1.5-2.9A2 2 0 0 1 9.24 10H20a2 2 0 0 1 1.94 2.5l-1.54 6a2 2 0 0 1-1.95 1.5H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h3.9a2 2 0 0 1 1.69.9l.81 1.2a2 2 0 0 0 1.67.9H18a2 2 0 0 1 2 2v2" /></svg></div><div class="empty-state-title">কোনো PDF নেই</div></div>';
        return;
    }
                ...
                try {
                    ...
                    <div class="pdf-card">
                        <div class="pdf-card-top">
                            ...
                            <div class="pdf-card-actions">
                                <button class="act-btn act-toggle" title="${p.is_premium ? 'Free করো' : 'Premium করো'}" onclick="mbTogglePremium(${p.id}, ${!p.is_premium})">${p.is_premium ? '<svg class="emoji-icon" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="#FBBF24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline;vertical-align:-0.125em"><path d="M11.525 2.295a.53.53 0 0 1 .95 0l2.31 4.679a2.123 2.123 0 0 0 1.595 1.16l5.166.756a.53.53 0 0 1 .294.904l-3.736 3.638a2.123 2.123 0 0 0-.611 1.878l.882 5.14a.53.53 0 0 1-.771.56l-4.618-2.428a2.122 2.122 0 0 0-1.973 0L6.396 21.01a.53.53 0 0 1-.77-.56l.881-5.139a2.122 2.122 0 0 0-.611-1.879L2.16 9.795a.53.53 0 0 1 .294-.906l5.165-.755a2.122 2.122 0 0 0 1.597-1.16z" /></svg>' : '<svg class="emoji-icon" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="#22C55E" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline;vertical-align:-0.125em"><rect width="18" height="11" x="3" y="11" rx="2" ry="2" /> <path d="M7 11V7a5 5 0 0 1 9.9-1" /></svg>'}</button>
                                <button class="act-btn act-edit" title="MCQ সম্পাদনা" onclick="mbOpenMcqPanel(${p.id}, '${esc(p.title)}', '${esc(p.file_url)}')"><svg class='emoji-icon' viewBox='0 0 24 24' width='1em' height='1em' fill='none' stroke='#F59E0B' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' style='display:inline;vertical-align:-0.125em'><path d='M12.659 22H18a2 2 0 0 0 2-2V8a2.4 2.4 0 0 0-.706-1.706l-3.588-3.588A2.4 2.4 0 0 0 14 2H6a2 2 0 0 0-2 2v9.34' /> <path d='M14 2v5a1 1 0 0 0 1 1h5' /> <path d='M10.378 12.622a1 1 0 0 1 3 3.003L8.36 20.637a2 2 0 0 1-.854.506l-2.867.837a.5.5 0 0 1-.62-.62l.836-2.869a2 2 0 0 1 .506-.853z' /></svg></button>
                                <button class="act-btn act-delete" title="মুছুন" onclick="mbDeletePdf(${p.id}, '${esc(p.title)}')"><svg class='emoji-icon' viewBox='0 0 24 24' width='1em' height='1em' fill='none' stroke='#EF4444' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' style='display:inline;vertical-align:-0.125em'><path d='M10 11v6' /> <path d='M14 11v6' /> <path d='M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6' /> <path d='M3 6h18' /> <path d='M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2' /></svg>️</button>
                            </div>
                        </div>
...
function mbOpenMcqPanel(pdfId, pdfTitle, pdfUrl) {
    ...
    if (ctxEl) {
        if (subName && chName) {
            ctxEl.innerHTML = '<svg class="emoji-icon" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="#3B82F6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline;vertical-align:-0.125em"><path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H19a1 1 0 0 1 1 1v18a1 1 0 0 1-1 1H6.5a1 1 0 0 1 0-5H20" /></svg> ' + esc(pdfTitle) + ' &nbsp;|&nbsp; <svg class="emoji-icon" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="#6366F1" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline;vertical-align:-0.125em"><path d="M12 7v14" /> <path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z" /></svg> ' + esc(subName) + ' › <svg class="emoji-icon" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="#6366F1" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline;vertical-align:-0.125em"><path d="M12 7v14" /> <path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z" /></svg> ' + esc(chName);
        ...
        <div style="background:var(--card);border:1px solid var(--border);border-radius:var(--radius);padding:14px;margin-bottom:10px">
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;margin-bottom:8px">
                ...
                <div style="display:flex;gap:6px;flex-shrink:0">
                    <button class="act-btn act-edit" onclick="mbEditMcq('${m.id}')" title="সম্পাদনা"><svg class='emoji-icon' viewBox='0 0 24 24' width='1em' height='1em' fill='none' stroke='#F59E0B' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' style='display:inline;vertical-align:-0.125em'><path d='M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z' /> <path d='m15 5 4 4' /></svg>️</button>
                    <button class="act-btn act-delete" onclick="mbDeleteMcq('${m.id}')" title="মুছুন"><svg class='emoji-icon' viewBox='0 0 24 24' width='1em' height='1em' fill='none' stroke='#EF4444' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' style='display:inline;vertical-align:-0.125em'><path d='M10 11v6' /> <path d='M14 11v6' /> <path d='M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6' /> <path d='M3 6h18' /> <path d='M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2' /></svg>️</button>
                </div>
            </div>
