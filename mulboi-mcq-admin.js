...
const MB_EXP_BOX_RULE = (
    ...
    `এছাড়া "line_box" নামে আরেকটি object দিবে — শুধুমাত্র সেই নির্দিষ্ট লাইন/বাক্যের bounding box ` +
    `যেখান থেকে সরাসরি এই প্রশ্নের উত্তর/মূল তথ্যটি এসেছে (পুরো প্যারা/টপিক না, শুধু ঐ এক বা দুই লাইন)। ` +
    `অত্যন্ত গুরুত্বপূর্ণ: line_box-এর y ও h হবে exp_box-এর ভেতরে আপেক্ষিক (relative) শতকরা হার (0-100), ` +
    `পুরো পেইজের সাপেক্ষে না। অর্থাৎ line_box.y=0 মানে exp_box-এর একদম উপরের প্রান্ত, line_box.y=100 মানে exp_box-এর ` +
    `একদম নিচের প্রান্ত। উদাহরণ: exp_box যদি পুরো প্যারার হয় আর উত্তরের লাইনটা প্যারার মাঝামাঝি থাকে, তাহলে line_box.y≈45, h≈10 এর মতো হবে। ` +
...
async function mbCropExplanationImage(pageNum, box, lineBox) {
    ...
    try {
        ...
        if (Number.isFinite(ly) && Number.isFinite(lh) && lh > 0) {
            // bug fix (root cause of "line highlight ultapalta jaygay"): line_box আগে raw
            // page-absolute % হিসেবে নেওয়া হতো, কিন্তু exp_box ink-scan দিয়ে shift/snap হয়ে
            // যায় (boxTopAbs/boxBottomAbs আসল AI y,h থেকে ভিন্ন হতে পারে) — ফলে line_box এর
            // সাথে exp_box এর sync থাকত না, highlight ভুল জায়গায় পড়ত। এখন line_box.y/h কে
            // exp_box এর ভেতরে আপেক্ষিক (relative 0-100%) ধরে নেওয়া হচ্ছে এবং সবসময় corrected
            // boxTopAbs/boxBottomAbs এর সাপেক্ষেই absolute position বের করা হচ্ছে — তাই ink-scan
            // যতই shift করুক, highlight সবসময় exp_box এর সঠিক আপেক্ষিক জায়গাতেই থাকবে।
            const boxTopAbs = py + mbBoxTopInCrop;
            const boxBottomAbs = py + mbBoxBottomInCrop;
            const boxSpan = boxBottomAbs - boxTopAbs;
            const lineTopAbs    = boxTopAbs + Math.max(0, Math.min(100, ly)) / 100 * boxSpan;
            const lineBottomAbs = boxTopAbs + Math.max(0, Math.min(100, ly + lh)) / 100 * boxSpan;
